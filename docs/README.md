...
```
In this example the reaction mixture sample (2.5 mL) is transferred to the benchtop NMR to perform 19F analysis and calculate the reaction outcome. For more examples check **here** and for additional parameters of the `FinalAnalysis` step -> **here**.
...
```
Full description of parameters is given **below**.
...
6. Special steps (i.e. `ConstrainedStep`) are updated.
#### 2. Execution
...
3. When the list of steps to execute is exhausted, the ODS checks for the reagent flasks and wastes volumes, loads the data (parameters and reaction outcome) to the algorithm class and updates the XDL object (see **preparation** above).
4. If any termination criteria is met, the optimization stops.
5. If control experiment is required, the parameters for that are taken from any of previously performed experiments and analyzed *specially*.
#### 3. Data management
...
`reference_step` - previous step in the procedure, with its properties. Used for additional sample preparations (e.g. dissolving solid after `Filter` step). *Reserved for future use*.
...
`sample_excess_volume` - excess of the sample volume to prime the pump, 2 mL by default.
...
`dilution_solvent_vessel` - name of the vessel for the solvent to dilute the vessel (given internally).
...
`shimming_solvent_flask` - name of the flask for the solvent for the shimming (given internally). The solvent is selected from the list of allowed solvents, if none found - exception is raised.
...
- `target` - indicates the calculation for the reaction outcome. Available:
	...
	- `novelty` - novelty, by default is the number of peaks identified. Special implementation for the NMR spectrum.
...
The algorithm implementation is based on the scikit-optimize python library and provides a simple and efficient library to minimize expensive and noisy black-box function. The parameters for the underlying algorithm are listed in the official documentation.
**4.	Design of Experiments**
Several experimental designs are available via a wrapper for pyDOE. Available designs include full and fractional factorial designs, Response-surface designs, and randomized designs. The wrapper calls the appropriate pyDOE2 functions to generate the design matrix and maps the levels onto the appropriate parameter values based on the search space bounds.
**5.	Dummy methods**
